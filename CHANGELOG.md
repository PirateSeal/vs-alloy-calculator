...
- **"Crawled – currently not indexed" signal** — The pre-JS HTML body was effectively empty (`<div id="root"></div>` plus a short `<noscript>` block), giving Googlebot's HTML-first quality pass almost no content to weigh. Each built page now ships a localized `<main class="seo-prerender">` inside `#root`, containing a real `<h1>`, the hero paragraph, and (on the overview route) the "how it works", "supported alloys", "why use it", and FAQ sections sourced from `SEO_CONTENT`. React's `createRoot(...).render(...)` replaces these children on mount, so users never see them, but Googlebot, social-preview crawlers, and no-JS visitors get substantive indexable copy per locale and per route.
- **Sitemap "Temporary processing error"** — The deploy workflow now uploads `sitemap*.xml` with an explicit `Content-Type: application/xml; charset=utf-8` (and `robots.txt` with `text/plain; charset=utf-8`), removing the risk of S3 falling back to `application/octet-stream` or `text/html` when `aws s3 sync` runs with a `--cache-control` override, which is the most common cause of the Search Console sitemap processing error.
...
- **WebApplication JSON-LD trimmed** — Dropped the `"browserRequirements": "Requires JavaScript"` field from both the base `index.html` JSON-LD and the per-route schema emitted by `getSeoContent`. The field was flagged by several SEO audits as a low-content-value signal without adding useful semantic information for Google.
